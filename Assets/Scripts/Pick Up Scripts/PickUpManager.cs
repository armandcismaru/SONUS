    ...
    /* The moment you start the game, the component 
     * from the PickUpManager is asked if the Canvas exists in the Start() routine.*/

    public class PickUpManager : MonoBehaviour {

        /* PickUpComponent is the camera, thus has the Canvas 
         * Each component from the ones that are attached to the player 
         * (e.g: Health, Supply etc.) are added to the "PickUpComponent" list. */

        [SerializeField]
        private List<PickUpComponent> instancePickupComponents;
        ...
        void Start()
        {
            ...
            if (uiComponent == null) throw new MissingComponentException(
         ...
         * script to all instances of pick up components 
         * that are already attached to the pick up manager.*/

        private void OnCollisionEnter(Collision collision)
        {
            if (collision.gameObject.tag == "PickUp")
            {
                /* Pass pick up component and script to Trigger; 
                 * the logic from the respective pick up item is being passed
                could change the Pick Up Type form inside Unity
                (Supply/Health/Harmour) */
